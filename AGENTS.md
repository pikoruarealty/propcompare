...
**Comparison depth is gated behind sign-in (owner direction, 2026-09-22 — reverses the earlier "no sign-in to compare" rule; see `DECISIONS.md`).** A comparison's column identity (photo, name, locality, developer, the chosen unit type) and its differences-first summary are open to everyone; the detailed row groups beneath them render locked until the buyer signs in with their phone number. Guided intake is the site's front door, not a nav item — it leads straight into matched properties a buyer can open or add to a comparison directly.
