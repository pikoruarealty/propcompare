...
## Developer analytics boundary

A developer never reads raw analytics events. The release job (`bun run analytics:release`, `src/lib/analytics/release.ts`) reads them as the application role and writes only thresholded aggregates (a figure needs at least 5 distinct identified visitors; a withheld figure has no number, never zero) to `developer_analytics_released`, `_pairings` and `_benchmarks`, tracked by `developer_analytics_runs`. Developer code reads those four tables through the `propcompare_developer_reader` role, which the database refuses everything else (raw events, the catalog, accounts, `private`, every write), so a slip fails in Postgres and does not leak. Which developer is asking comes from the session, and every query filters by it. See `docs/schema/schema.v21.md`, `v23`, `v24`.
