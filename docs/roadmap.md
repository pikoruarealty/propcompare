...
**Status update 2026-09-25:** the enquiry flow is now admin-first (the admin forwards an enquiry to the developer or closes it; schema v19), units per floor is the whole floor's, and the price categorization was verified against the live data. Everything in the acceptance below is built. What keeps Phase 3 from a clean close is decisions, not code: comparison analytics (slice 4) needs the owner's answers on identity, retention, consent and what counts as a choice, and the landing page's richer content needs the owner's scope.
