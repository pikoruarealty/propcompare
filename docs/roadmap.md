...
**Area of focus: Bhavarth** — all backend for this phase: the discovery/comparison budget-range matching service (the only code path with a service-role connection into `private`) and the four remaining buyer API routes (`saved-properties`, `comparisons`, `enquiries`, `dossier-unlocks`). See [the matching-service tasklist](tasklists/2026-09-01-phase-3-budget-range-matching.md) and [the discovery/matches endpoint tasklist](tasklists/2026-09-18-discovery-matches-endpoint.md).

**Status (Bhavarth's piece): matching service and its endpoint complete 2026-09-18.** `src/lib/matching/budget-range.ts` implements the inclusive `[min × 0.80, max × 1.20]` matcher against `private.unit_price_history`, and `POST /api/v1/discovery/matches` (`src/app/api/v1/discovery/matches/route.ts`) wires it into a stateless, price-free buyer response. Boundary, role-denial, and no-leak behavior are verified by database-backed tests; see `PROGRESS.md`'s 2026-09-18 entries. `saved-properties`, `comparisons`, `enquiries`, and `dossier-unlocks` are not yet built — see the 2026-09-18 `DECISIONS.md` entry on why backend ownership for this phase sits entirely with Bhavarth rather than being split with Deep as this line previously implied.
...
**Status (Deep's piece): the intake→matches wiring is complete 2026-09-18** ([its tasklist](tasklists/2026-09-18-intake-matches-ui.md)). `/intake`'s summary step now POSTs the stated range and renders matched properties in place — inside `/intake`, because no mechanism available today could carry the range to a separate address. The remaining three slices (comparison, saved properties, the OTP gate, enquiry submission) are blocked on the four routes that do not exist yet. One follow-up is open against Bhavarth: the endpoint accepting an unbounded upper end, so the slider's "₹5 crore or more" is bounded by the catalog's highest published price rather than by the stated figure — see the 2026-09-18 `DECISIONS.md` entry.
