...
- [ ] **Quarterly RERA refresh worker (built, off by default).** It reads GujRERA by itself, so it only runs with `RERA_WORKER_ENABLED=true` in the web server or as one `bun run rera:worker` process (never both on several hosts: a row lock stops double checks, but extra workers are wasteful). GujRERA's terms were read on 2026-09-20 (`DECISIONS.md`): nothing forbids it, with two soft items still open: show "Source: GujRERA, checked on …" wherever RERA facts reach buyers, and ask `inforera@gujarat.gov.in` for written confirmation because the Copyright and Disclaimer pages disagree on whether permission is needed. Re-read the terms before turning it on. Also confirm the request spacing (`RERA_WORKER_SPACING_MS`, 15 s) is acceptable, and add monitoring: a check that fails repeatedly is only visible today as failed `rera_fetch_jobs` rows. Completed projects that no longer file are re-checked weekly indefinitely (one request each).
