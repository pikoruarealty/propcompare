...
- [ ] **Email delivery for developer invites** (set-password link) and admin notifications. The invite flow is built; today the owner copies the one-time link from the team panel and sends it by hand. Wire an email provider so the same link is emailed, and add rate limiting on the accept page.
