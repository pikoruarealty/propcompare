...
- [ ] **Developer analytics release job** scheduled daily after `analytics:purge` (schema v21, Phase 4 Part 2; the job is not written yet). Without it developers see no figures, and a stale run must show as stale (2026-09-26 — Deep).
...
- [ ] **Analytics retention job** scheduled daily (`bun run analytics:purge`): raw events older than 13 whole months are rolled into monthly counts and deleted (`DECISIONS.md` 2026-09-25). Without the schedule, raw rows are kept longer than the privacy policy will say.
...
- [ ] **Terms of service and privacy policy** (owner decision 2026-09-21: written when development is complete, just before deployment, with counsel). The privacy policy must describe what is actually collected (intake answers, enquiries, phone numbers for OTP, saved properties, analytics). For analytics it must name: the two first-party cookies `pc_vid` (a random visitor id, about 13 months) and `pc_sid` (a visit, 30 minutes); the events recorded (pages and properties viewed, comparisons and what was in them, sections opened, unit types and focus chosen, visible time on a page, saves, enquiries, intake choices); the coarse budget band of the ceiling stated in intake; campaign tags and the referring domain; device class; that no name, phone, email, IP or price is in them; that raw events are kept 13 months and then only counts; and that Global Privacy Control and Do Not Track switch recording off (`docs/schema/schema.v20.md`); link both from the footer; the signup and enquiry forms must reference them.
