...
| `GET /api/v1/developer/properties/{id}`    | Implemented (2026-09-26) | Developer staff        | The same `window`. One of the developer's own listed properties: `figures`, the counted `splits` (visitors, viewers by budget band and device, comparers by budget band; a split appears only where something was counted), `rivals` (at most five listed properties opened in a comparison beside it by at least 5 identified visitors: `{ property, developerName, own, visitors }`, most compared first, never the rival's own figures), `benchmarks` (`{ figure, cohort, cohortProperties, cohortDevelopers, median }`, present only when the cohort is large enough and the median meets the gate) and `completeness`. Another developer's property, an unlisted one, an unknown one and a malformed id are all `404 property_not_found`.                          |
