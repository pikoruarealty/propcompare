...
A buyer's comparisons and their ordered property/unit items. Saving the same comparison again (the same properties and unit types, in any order) returns the one already saved rather than a duplicate (2026-09-21). Creates and reads only — no `PATCH`/item-mutation route; a comparison is created once with its full item list.
...
| `PATCH /api/v1/admin/enquiries/{id}`                           | Implemented (2026-09-21)                    | Admin      | Body `{ status }`, one of `new`, `contacted`, `closed`. Moves a buyer's enquiry through the admin inbox at `/admin/enquiries` (server-rendered, so there is no list route). Any admin; it publishes nothing. `404` `enquiry_not_found` for an unknown or malformed id, `422` for a bad status.                                                                                                            |
