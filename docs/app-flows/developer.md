...
**2026-09-25 — Deep:** step 4 is now planned as a developer-facing, aggregates-only view over the schema v20 analytics (`docs/tasklists/2026-09-25-phase-4-developer-analytics.md`). The developer is resolved from `requirePortalRole("developer", …)`, never from request input. Only the linked developer's own properties appear, every buyer-behaviour figure is withheld below a privacy threshold (shown as "not enough data", never zero), and no visitor/session id, price, private bucket or buyer detail is shown. The threshold, competitor-pairing visibility, the enquiry count's definition and property eligibility are open choices in that tasklist. The developer's view of a forwarded enquiry (schema v19) is not part of it.
