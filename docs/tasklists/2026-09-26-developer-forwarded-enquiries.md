...
1. [ ] `src/lib/developers/enquiry-inbox.ts` (or similar): a query scoped to one `developerId` (from `requirePortalRole("developer", …).role.developerId`, never request input), returning only `status = 'forwarded'` rows for that developer's properties — name, phone, email, message, property, unit type, `forwardedAt`. Mirrors `listEnquiryInbox`'s shape and join, narrowed by developer and status.
2. [ ] A page under `src/app/developers/(portal)/enquiries/`, replacing nothing else in the holding shell. Empty state for no forwarded enquiries yet.
3. [ ] Update the stale comment in `src/lib/buyer/enquiry-inbox.ts` ("the buyer's number... never reaches a buyer or developer surface") — it now does, deliberately, once forwarded.
4. [ ] `docs/product/privacy-policy-inputs.md`: record that a forwarded enquiry's buyer contact (name, phone, message) is shown to that property's developer, and that this is the first developer-facing use of the phone number.
5. [ ] Tests: two developers cannot see each other's enquiries or properties; an unforwarded or closed enquiry is absent; a revoked or wrong-role session is refused; the query never returns another developer's row even by a guessed id.
6. [ ] `docs/app-flows/developer.md`, `docs/api/api-spec.v1.md` if a route is added, `PROGRESS.md`.
