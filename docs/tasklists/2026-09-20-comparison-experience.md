...
**Status:** slice 1 in progress 2026-09-20
...
- [ ] `src/lib/compare/`: the comparison model (pure): like-for-like unit type choice, ordered row definitions, normalised difference detection, differences-only, size bars, the rule-built "If you choose A over B" summary, honest missing states. Unit-tested on Kimana-shaped data and edge cases.
- [ ] Read path: several published dossiers by slug in one call (unlisted, deleted and unknown slugs are simply absent).
- [ ] `/compare?p=…&v=…` page: sticky header, summary block, controls, grouped sections, empty and one-property states, phone two-up.
- [ ] "Compare" toggle on property cards and the dossier; the bottom tray; selection kept in the browser and reflected in the address.
- [ ] Tests: model, page, toggle and tray components, no price anywhere (the existing leak guard on the compared data), a real-database check of the read path.
- [ ] Real-browser check on Kimana plus a second published property at desktop and phone width.
