...
- **Brochure/media storage strategy (GCS).** Flagged as an open, expensive-to-reverse gate in the 2026-09-07 `DECISIONS.md` entry on the dossier media gate: public bucket vs. signed URLs vs. a proxy route, for both `source_documents` (brochure originals, never buyer-facing) and `property_media`. No GCS SDK is installed and no credentials are configured (`.env.example` has the four `GCS_*` variables, all blank). This blocks step 2's brochure upload literally — there is nowhere to put the file yet. Needs a decision before that step starts, not an assumption baked into upload code.
