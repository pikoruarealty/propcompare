...
- **Storage abstraction and provider.** Flagged as an open, expensive-to-reverse gate in the 2026-09-07 `DECISIONS.md` entry on the dossier media gate: public bucket vs. signed URLs vs. a proxy route, for both `source_documents` (brochure originals, never buyer-facing) and `property_media`. Corrected from an earlier draft of this tasklist: the `@google-cloud/storage` SDK is already installed and already used, but only for one narrow, read-only, GCS-specific path — `src/lib/ocr/source-loader.ts`'s `createGcsSourcePdfLoader`, which downloads a brochure PDF for the OCR pipeline via a direct, uninterfaced call to the SDK. **Nothing uploads to any storage anywhere in this codebase.** The user is separately evaluating moving hosting off GCP to a Hostinger VPS and wants a storage interface the app talks to generically (upload/delete/resolve-a-URL), backed by a swappable provider adapter — analogous to the existing OCR-provider adapter pattern (`src/lib/ocr/adapter.ts`). This decision blocks step 2's brochure upload literally — there is nowhere to put a file yet, and no interface shape has been agreed. Needs resolving before that step starts, not an assumption baked into upload code. When it lands, `source-loader.ts`'s existing download call should very likely be rewritten to go through the same interface rather than staying a second, parallel direct-SDK path.
...
- [ ] Brochure upload UI + storage (blocked on the GCS decision above).
