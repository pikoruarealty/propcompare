...
- **Public trust links** on every property: "Report a problem / request removal". Not built as a queue; see the placeholder dialog.
...
- [ ] Page-routing confirmation — grid, selection and zoomable viewer done and verified in Chrome, Brave and Edge. Categories now come from a vision-model router, not text scanning (`DECISIONS.md` 2026-09-20). Remaining, in order:
  - [x] Router adapter (`src/lib/ocr/page-router.ts`, Gemini 2.5 Flash via OpenRouter, native PDF, windows under a byte budget): per-page category, confidence, imagery tags, optional floor-plan caption hint; 15 mocked-fetch tests. **Run live 2026-09-20** on Kimana, Amaris and 360: accurate per the owner's manual check, about $0.027 in total (`DECISIONS.md` 2026-09-20).
